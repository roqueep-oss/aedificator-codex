...
- **Privacidade do instalador**: uma instalação limpa **não embute nenhuma chave de provedor** — o app inicia com todas em branco e cada usuário cadastra as próprias em *Configurações*. `config.json`, `.env`, `token_usage.json` e logs do desenvolvedor são excluídos do pacote (`build.files`), conferidos pelo guard `node scripts/check-dist.js` (roda após `build:*`/`dist`) e pelo smoke test da CI, que **falha o build se qualquer chave aparecer** no `config.json` gerado.
